...
// Creating shortcut for an integer pair //iPair contains a pair of nodes (u,v) which create an edge
...
struct Graph
{
    ...
    vector< pair<int, iPair> > edges;	//array of pairs
...
struct DisjointSets
{
    ...
    DisjointSets(int n)
    {
        // Allocate memory
        this->n = n;
        parent = new int[n+1]; //parent is the pointer to an array of size n+1
        rnk = new int[n+1];
...
int main()
{
    ...
    //  making above shown graph //u,v,weight
    g.addEdge(0, 1, 4);
    g.addEdge(0, 7, 8);
    g.addEdge(1, 2, 8);
    g.addEdge(1, 7, 11);
